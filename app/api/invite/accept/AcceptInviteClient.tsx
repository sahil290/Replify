...
export default function AcceptInviteClient() {
  ...
  async function checkAuthThenAccept() {
    ...
    if (!user) {
      setStatus('login')
      setMessage('Sign in or create an account to accept this invite.')
      return
    }
    setStatus('loading')
    await doAccept()
  ...
  async function doAccept() {
    try {
      ...
      setStatus('success')
      setMessage(data.already_member ? "You're already a member!" : 'You have successfully joined the workspace!')
      ...
      <div className="bg-white border border-gray-200 rounded-2xl shadow-lg p-10 w-full max-w-md text-center">
        ...
        {(status === 'checking' || status === 'loading') && (
          <>
            ...
            <h1 className="text-xl font-bold text-gray-900 mb-2">
              {status === 'checking' ? 'Checking your invite…' : 'Joining workspace…'}
            </h1>
        ...
        {status === 'login' && (
          <>
            ...
            <p className="text-sm text-gray-500 mb-6">{message}</p>
            <div className="flex flex-col gap-3">
              <Link href={`/auth/login?redirect=${encodeURIComponent(`/invite/accept?token=${token}`)}`} className="btn-primary w-full justify-center">
                Sign in
              </Link>
              <Link href={`/auth/signup?redirect=${encodeURIComponent(`/invite/accept?token=${token}`)}`} className="btn-ghost w-full justify-center">
                Create account
              </Link>
            </div>
          </>
