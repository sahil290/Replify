...
export async function GET() {
  try {
    ...
    const pct = limit === Infinity ? 0 : Math.min(100, Math.round(((usage?.analyze_requests ?? 0) / limit) * 100));
    ...
    const avgLatency = logs?.length ? Math.round(logs.filter((l) => l.latency_ms).reduce((s, l) => s + (l.latency_ms ?? 0), 0) / logs.length) : null;
    ...
    return NextResponse.json({
      ...
      daily,
      reset_date: new Date(new Date().getFullYear(), new Date().getMonth() + 1, 1).toLocaleDateString("en-US", {month: "long", day: "numeric"}),
    });
  } catch (err: any) {
